...
const CIIU_PRESETS: CIIUPreset[] = [
  {
    code: "5610",
    name: "Restaurantes y servicios móviles de comida",
    flags: { handlesFood: true, hasDischarges: true, hasPublicPremises: true },
  },
  {
    code: "5511",
    name: "Alojamiento en hoteles",
    flags: { isTourismProvider: true, hasPublicPremises: true },
  },
  {
    code: "4791",
    name: "Comercio por Internet (e-commerce)",
    flags: { hasPublicPremises: false },
  },
  {
    code: "4711",
    name: "Comercio minorista no especializado",
    flags: { hasPublicPremises: true },
  },
];
...
function WizardProgressBar({
  ...
  steps: readonly { key: string; title: string; icon: React.ElementType }[];
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.key} className="flex-1 text-center">
              <div
                className={`mx-auto w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 ${
                  isActive
...
const LS_CHECKLIST = "formalizationChecklist"; 
const LS_PROGRESS = "formalizationProgress";   
...
export default function FormalizationWizard() {
  ...
  const savedProgress =
    typeof window !== "undefined" ? loadJSON<{ stepIndex?: number; checklistStates?: Record<string, RequirementState> } | null>(LS_PROGRESS, null) : null;

  const initialStep = (() => {
    if (!savedProgress) return 0;
    if (wantsResume) return 3; 
    return Math.min(Number(savedProgress.stepIndex ?? 0), 3);
  })();
  ...
  const [checklistStates, setChecklistStates] = useState<Record<string, RequirementState>>(
    (savedProgress && savedProgress.checklistStates) || {}
  );
  ...
  const groupedChecklist = useMemo(() => {
    ...
    Object.values(grouped).forEach((group) =>
      group.sort((a, b) => Number(!!a.optional) - Number(!!b.optional))
    );
    return grouped;
  ...
  const stepErrors = useMemo(() => {
    ...
    if (stepIndex === 0) {
      if (!bizName || bizName.trim().length < 3)
        e.bizName = "Ingresa un nombre de negocio (mín. 3 caracteres).";
      if (!city || city.trim().length < 3) e.city = "Ingresa la ciudad.";
    }
    if (stepIndex === 1) {
      if (hasPublicPremises && expectedAforo < 1)
        e.expectedAforo = "Si tienes local, el aforo debe ser al menos 1.";
    }
    return e;
  ...
  useEffect(() => {
    ...
    const hasChecklist =
      Array.isArray(loadJSON(LS_CHECKLIST, [])) &&
      loadJSON(LS_CHECKLIST, []).length > 0;
    if (hasChecklist) setStepIndex(3);
  }, [wantsResume]);

  useEffect(() => {
    saveJSON(LS_PROGRESS, { stepIndex, checklistStates });
  }, [stepIndex, checklistStates]);
  ...
  const handleFinish = () => {
    ...
    try {
      saveJSON(LS_CHECKLIST, dataToStore);
      saveJSON(LS_PROGRESS, { stepIndex: 3, checklistStates });
    } catch (error) {
      console.error("Error al guardar el checklist en localStorage:", error);
      alert("Error al guardar el checklist, por favor revisa tus respuestas");
      return;
    }
      ...
      <div className="container mx-auto px-4 sm:px-6">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-3xl mx-auto">
            Este asistente inteligente te guiará paso a paso para generar una
            lista de requisitos a tu medida. ¡Empecemos!
          </p>
        </div>
        ...
        <div className="max-w-4xl mx-auto bg-white p-6 md:p-10 rounded-2xl shadow-xl border border-gray-200/80">
          ...
          <div className="transition-all duration-300">
              ...
              <div>
                ...
                <FormField label="Nombre de tu Negocio o Emprendimiento" error={errors.bizName}>
                  <input
                    ...
                    placeholder="Ej: Artesanías El Caporal"
                ...
                <div className="grid md:grid-cols-2 gap-x-6">
                  ...
                  <FormField label="Tipo de Persona">
                    <select
                      value={legalForm}
                      onChange={(e) =>
                        setLegalForm(e.target.value as Flags["legalForm"])
                      }
                      className="w-full rounded-lg border border-gray-300 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-orange-300 focus:border-orange-400"
                    >
            ...
            {stepIndex === 1 && (
              <div>
                ...
                <div className="space-y-5">
                  ...
                  {hasPublicPremises && (
                    <FormField
                      label="Aforo Estimado del Local"
                      error={errors.expectedAforo}
                    >
                      <input
                        type="number"
                        value={expectedAforo}
                        onChange={(e) => setExpectedAforo(Number(e.target.value))}
                        className={`w-full rounded-lg border px-4 py-2.5 focus:outline-none focus:ring-2 ${
                          errors.expectedAforo
                            ? "border-red-500 ring-red-200"
                            : "border-gray-300 focus:ring-orange-300 focus:border-orange-400"
                        }`}
                      />
                  ...
                  <label className="flex items-center p-4 border rounded-lg hover:bg-gray-50 transition cursor-pointer">
                    <input
                      type="checkbox"
                      checked={handlesFood}
                      onChange={(e) => setHandlesFood(e.target.checked)}
                      className="h-5 w-5 rounded text-orange-600 focus:ring-orange-500"
                    />
                    <span className="ml-4 text-sm font-medium text-gray-800">
                      ¿Manipulas o preparas alimentos?
                    </span>
                  </label>
                  <label className="flex items-center p-4 border rounded-lg hover:bg-gray-50 transition cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setHasDischarges(e.target.checked)}
                      className="h-5 w-5 rounded text-orange-600 focus:ring-orange-500"
                    />
                    <span className="ml-4 text-sm font-medium text-gray-800">
                      ¿Generas vertimientos, emisiones o ruido significativo?
                    </span>
                  </label>
                  <label className="flex items-center p-4 border rounded-lg hover:bg-gray-50 transition cursor-pointer">
                    <input
                      type="checkbox"
                      checked={isTourismProvider}
                      onChange={(e) => setIsTourismProvider(e.target.checked)}
                      className="h-5 w-5 rounded text-orange-600 focus:ring-orange-500"
                    />
                    <span className="ml-4 text-sm font-medium text-gray-800">
                      ¿Eres un prestador de servicios turísticos (hotel, agencia, etc.)?
                    </span>
                  </label>
                </div>
              </div>
            ...
            {stepIndex === 2 && (
              <div>
                <h3 className="text-2xl font-bold text-green-800 mb-6">
                  3. Sobre tu Operación Contable y de Datos
                </h3>
                <div className="space-y-5">
                  <label className="flex items-center p-4 border rounded-lg hover:bg-gray-50 transition cursor-pointer">
                    <input
                      type="checkbox"
                      checked={isElectronicInvoicingObliged}
                      onChange={(e) =>
                        setIsElectronicInvoicingObliged(e.target.checked)
                      }
                      className="h-5 w-5 rounded text-orange-600 focus:ring-orange-500"
                    />
                    <span className="ml-4 text-sm font-medium text-gray-800">
                      ¿Estás obligado a emitir Factura Electrónica?
                    </span>
                  </label>
                  <label className="flex items-center p-4 border rounded-lg hover:bg-gray-50 transition cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        setProcessesPersonalData(e.target.checked)
                      }
                      className="h-5 w-5 rounded text-orange-600 focus:ring-orange-500"
                    />
                    <span className="ml-4 text-sm font-medium text-gray-800">
                      ¿Almacenas y tratas datos personales de clientes o empleados?
                    </span>
                  </label>
                  {processesPersonalData && (
                    <label className="flex items-center p-4 border rounded-lg hover:bg-gray-50 transition cursor-pointer">
                      <input
                        type="checkbox"
                        checked={meetsSICThresholds}
                        onChange={(e) => setMeetsSICThresholds(e.target.checked)}
                        className="h-5 w-5 rounded text-orange-600 focus:ring-orange-500"
                      />
                      <span className="ml-4 text-sm font-medium text-gray-800">
                        ¿Tus activos totales superan las 100,000 UVT? (Requerido para RNBD)
                      </span>
                    </label>
                  )}
                </div>
              ...
              <div>
                <div className="text-center mb-8">
                  <h3 className="text-2xl font-bold text-green-800">
                    4. Tu Checklist de Formalización Personalizado
                  </h3>
                  <p className="text-gray-600 mt-2">
                    Basado en tus respuestas, estos son los requisitos que te aplican. ¡Manos a la obra!
                  </p>
                </div>
                ...
                <div className="space-y-8">
                  {Object.entries(groupedChecklist).map(([category, items]) => (
                    <div key={category}>
                      <h4 className="text-lg font-bold text-green-700 mb-4 pb-2 border-b-2 border-green-200">
                        {category}
                      </h4>
                      <ul className="space-y-4">
                        {items.map((item) => (
                          <li
                            key={item.key}
                            className="bg-white rounded-lg border border-gray-200 p-4 transition hover:shadow-md hover:border-green-300"
                          >
                            ...
                            {(item.renewalHint || item.links) && (
                              <div className="mt-3 pt-3 border-t border-gray-100">
                                {item.renewalHint && (
                                  <p className="text-xs text-gray-500">
                                    <strong>Renovación:</strong> {item.renewalHint}
                                  </p>
                                )}
                                {item.links && (
                            ...
                            <div className="mt-4 flex items-center gap-2">
                              <button
                                onClick={() => setRequirementState(item.key, "approved")}
                                className="text-xs font-semibold rounded-md border border-gray-200 px-2.5 py-1.5 hover:bg-green-50 hover:border-green-300 transition-all"
                              >
                                Marcar Cumplido
                              </button>
                              <button
                                onClick={() => setRequirementState(item.key, "pending")}
                                className="text-xs font-semibold rounded-md border border-gray-200 px-2.5 py-1.5 hover:bg-amber-50 hover:border-amber-300 transition-all"
                              >
                                Marcar Pendiente
                              </button>
                              {item.optional && (
                                <button
                                  onClick={() => setRequirementState(item.key, "na")}
                                  className="text-xs font-semibold rounded-md border border-gray-200 px-2.5 py-1.5 hover:bg-gray-100 hover:border-gray-300 transition-all"
                                >
                                  No Aplica
                                </button>
                              )}
        ...
        <div className="max-w-4xl mx-auto mt-6 sticky bottom-4">
          <div className="bg-white/80 backdrop-blur-sm supports-backdrop-filter:bg-white/60 border rounded-xl p-3 flex items-center justify-between shadow-lg">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg font-bold py-2 px-4 text-gray-700 transition-all disabled:opacity-40 disabled:cursor-not-allowed hover:bg-gray-100"
            >
              <ArrowLeft size={16} />
              <span>Atrás</span>
            </button>
            ...
            {stepIndex < steps.length - 1 ? (
              <button
                onClick={handleNext}
                className="inline-flex items-center gap-2 rounded-lg font-bold py-2 px-4 bg-orange-500 text-white transition-all hover:bg-orange-600 disabled:opacity-40 disabled:cursor-not-allowed"
              >
                <span>Siguiente</span>
                <ArrowRight size={16} />
              </button>
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg font-bold py-2 px-4 bg-green-600 text-white transition-all hover:bg-green-700"
              >
                <span>Finalizar</span>
                <Check size={16} />
              </button>
